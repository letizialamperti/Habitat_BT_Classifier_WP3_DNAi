...
def calculate_class_weights_from_csv(protection_file: Path) -> torch.Tensor:
...
def main():
    ...
    class_weights = calculate_class_weights_from_csv(Path(args.protection_file))
    ...
    wandb_logger = WandbLogger(project='ORDNA_Class_july_binary', save_dir="lightning_logs", config=args, log_model=False)
